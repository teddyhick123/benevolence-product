...
export default async function Dashboard({ searchParams }: { searchParams?: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const [metaRes, roleRes, settingsRes, kpiResult] = await Promise.all([
    ...
    fetch(`${base}/api/portfolio/${portfolioId}/role`, {
      cache: 'no-store',
      headers: { cookie: cookieHeader }
    }).catch(() => null),
    fetch(`${base}/api/portfolio/${portfolioId}/settings`, {
      cache: 'no-store',
      headers: { cookie: cookieHeader }
    }).catch(() => null),
    (async () => {
  ...
  const roleJson: { role?: string; can_edit?: boolean } = roleRes?.ok
    ? await roleRes.json().catch(() => ({ role: 'viewer', can_edit: false }))
    : { role: 'viewer', can_edit: false };
  const canEdit = !!roleJson?.can_edit;

  const settingsJson: { show_map?: boolean; widgets?: string[] } = settingsRes?.ok
    ? await settingsRes.json().catch(() => ({ show_map: true, widgets: [] }))
    ...
    <div className="space-y-8 isolate w-full">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between w-full gap-4">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3 w-full sm:w-auto">
          ...
          <a
            href={"/api/portfolio/" + portfolioId + "/board-report"}
            download
            className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-md border border-azure text-azure hover:bg-azure/5 transition-all duration-200"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
