...
export default function GrantHealthDashboard({ portfolioId }: Props) {
  ...
  const getRiskBadge = (level: string) => {
    const colors: Record<string, string> = {
      ...
      medium: 'bg-sunset/15 text-ink border border-sunset/30',
      high: 'bg-red-50 text-red-700 border border-red-100',
    };
    return colors[level] || 'bg-neutral-100 text-neutral-700 border border-black/5';
  };

  const getPriorityColor = (priority: string) => {
    const colors: Record<string, string> = {
      overdue: 'text-red-700 bg-red-50 border border-red-100',
      urgent: 'text-ink bg-sunset/15 border border-sunset/30',
      normal: 'text-neutral-600 bg-neutral-50 border border-black/5',
    };
    return colors[priority] || 'text-neutral-600 bg-neutral-50 border border-black/5';
  };
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="rounded-2xl border border-black/5 bg-white shadow-soft p-6">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-neutral-500">Total Grants</span>
            <span className="text-2xl font-bold text-ink">{summary.totalGrants}</span>
          </div>
          <div className="mt-2 flex items-center gap-2 text-xs">
            {summary.highRisk > 0 && (
              <span className="px-2 py-0.5 rounded-full bg-red-50 text-red-700 border border-red-100">{summary.highRisk} high risk</span>
            )}
            {summary.mediumRisk > 0 && (
              <span className="px-2 py-0.5 rounded-full bg-sunset/15 text-ink border border-sunset/30">{summary.mediumRisk} medium</span>
            )}
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-black/5 bg-white shadow-soft p-6">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-neutral-500">Avg Health Score</span>
            <span className={`text-2xl font-bold ${
              summary.avgHealthScore >= 80 ? 'text-green-600' :
              summary.avgHealthScore >= 50 ? 'text-coral' : 'text-red-600'
            }`}>
        ...
        <div className="rounded-2xl border border-black/5 bg-white shadow-soft p-6">
          <div className="flex items-center justify-between">
            ...
            <span className="text-2xl font-bold text-ink">{formatCurrency(summary.totalDisbursed)}</span>
          </div>
          <p className="mt-2 text-xs text-neutral-500">Across all grants</p>
        </div>

        <div className="rounded-2xl border border-black/5 bg-white shadow-soft p-6">
          <div className="flex items-center justify-between">
            <span className="text-sm font-medium text-neutral-500">Attention Needed</span>
            <span className={`text-2xl font-bold ${summary.overdueItems > 0 ? 'text-red-600' : 'text-green-600'}`}>
              {summary.overdueItems}
            </span>
          </div>
          <p className="mt-2 text-xs text-neutral-500">Overdue milestones & reports</p>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Grant List */}
        <div className="lg:col-span-2 rounded-2xl border border-black/5 bg-white shadow-soft overflow-hidden">
          <div className="px-6 py-4 border-b border-black/5 flex items-center justify-between">
            <h2 className="text-lg font-semibold text-ink">Grants Overview</h2>
              ...
              <table className="min-w-full divide-y divide-black/5">
                <thead className="bg-neutral-50">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-neutral-500 uppercase tracking-wider">Disbursed</th>
                  </tr>
                </thead>
                <tbody className="bg-white divide-y divide-black/5">
                  {grants.map((grant) => (
                    <tr key={grant.grant_id} className="hover:bg-neutral-50">
                      <td className="px-6 py-4">
                        <div>
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${getRiskBadge(grant.risk_level)}`}>
                              {grant.risk_level}
                            </span>
                            {grant.grant_type && (
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm font-medium text-ink">{grant.health_score}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4">
        ...
        <div className="rounded-2xl border border-black/5 bg-white shadow-soft overflow-hidden">
          <div className="px-6 py-4 border-b border-black/5">
            ...
            <p className="text-sm text-neutral-500">Next 30 days</p>
          </div>
          {deadlines.length === 0 ? (
                ...
                <li key={`${deadline.entity_id}-${idx}`} className="px-6 py-4">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        <span className={`px-2 py-0.5 rounded-full text-xs font-medium capitalize ${getPriorityColor(deadline.priority)}`}>
                        ...
                        <span className="text-xs text-neutral-500 capitalize">{deadline.deadline_type}</span>
                      </div>
                      <p className="mt-1 text-sm font-medium text-ink">{deadline.title}</p>
                      <p className="text-xs text-neutral-500">{deadline.holding_name}</p>
                    </div>
                  </div>
                </li>
              ))}
