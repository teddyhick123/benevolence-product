...
function emptyResult(): InvitationEmailOutboxResult {
  return { scanned: 0, sent: 0, cancelled: 0, failed: 0, errors: [] };
}

async function deliverClaimedInvitationEmail(
  db: SupabaseClient,
  event: InvitationEmailEvent,
  result: InvitationEmailOutboxResult,
  dryRun: boolean,
): Promise<void> {
  result.scanned++;
  ...
  try {
    ...
    if (delivered) {
      ...
      // would schedule a retry and the invitee would be mailed twice.
      result.errors.push({
        eventId: event.id,
...
/**
 * Sends one just-created outbox event promptly. The conditional update is the
 * claim: a cron worker that gets there first simply wins and this call is a
 * no-op. Retry events stay exclusively with the scheduled worker.
 */
export async function deliverNewInvitationEmailOutboxEvent(
  db: SupabaseClient,
  eventId: string,
  ...
  const { data, error } = await db
    ...
    .update({ status: 'sending', attempts: 1, claimed_at: new Date().toISOString() })
    .eq('id', eventId)
    .eq('status', 'pending')
    .eq('attempts', 0)
  ...
  await deliverClaimedInvitationEmail(db, data as InvitationEmailEvent, result, false);
  return result;
}
...
export async function drainInvitationEmailOutbox(
  ...
  for (const event of (data ?? []) as InvitationEmailEvent[]) {
    await deliverClaimedInvitationEmail(db, event, result, options.dryRun ?? false);
  }
  return result;
}
