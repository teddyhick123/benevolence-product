...
export default function GrantAttentionQueue({ grants, loading, onNewGrant, orgId }: Props) {
    ...
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
        ...
        <div>
          ...
          <p className="text-sm text-neutral-500 mt-1">No {grantPlural} need immediate attention.</p>
        </div>
        {onNewGrant && (
      ...
      {sorted.length === 0 ? (
        <div className="rounded-2xl bg-green-50 border border-green-100 p-6 text-center">
          <p className="text-sm font-medium text-green-700">
            {mode === 'overdue' ? `No overdue ${grantPlural}` :
             mode === 'high_risk' ? `No high-risk ${grantPlural}` :
             `No ${grantPlural} need immediate action`}
          </p>
        </div>
      ) : (
                ...
                <div className="flex-1 min-w-0 pl-2">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    <Link href={`/dashboard/grants/${g.id}`} className="font-medium text-ink hover:text-azure truncate">
                      {g.holdings?.name ?? `Unnamed ${grantLabel}`}
                    </Link>
                    {styles && (
