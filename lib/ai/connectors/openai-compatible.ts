...
export function usage(value: OpenAICompatibleUsage | undefined): AIUsage | undefined {
...
export function stopReason(reason: string | null | undefined): string | null {
...
export function normalizedError(status: number): AIExecutionError {
...
export function toolDefinitions(tools: ToolDefinition[]) {
...
export function messages(system: string | undefined, input: AIMessage[]) {
...
export type OpenAICompatibleConfig = {
  ...
  apiKey: string;
  fetch?: typeof fetch;
  /** Extra request headers, e.g. a provider's attribution headers. */
...
/**
 * The chat-completions wire format shared by OpenRouter and OpenAI. Everything
 * provider-specific lives in the config a subclass supplies — origin, headers,
 * and any extra top-level body fields.
 */
export abstract class OpenAICompatibleConnector implements AIConnector {
  abstract readonly id: AIConnectorId;
  ...
  private async send(body: Record<string, unknown>, signal?: AbortSignal): Promise<Response> {
    ...
    try {
      response = await this.fetcher(`${this.config.origin}/chat/completions`, {
        method: 'POST',
        headers: {
  ...
  private async complete(
    ...
    let value: OpenAICompatibleResponse;
    try {
      value = await response.json() as OpenAICompatibleResponse;
    } catch {
  ...
  private async *stream(
    ...
    const parseEvent = (line: string): OpenAICompatibleResponse | null => {
      ...
      while (true) {
        ...
        for (const line of lines) {
          ...
          const choice = event.choices?.[0] as {
            finish_reason?: string | null;
            delta?: { content?: string | null; tool_calls?: OpenAICompatibleToolCall[] };
          } | undefined;
