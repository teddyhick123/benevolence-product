...
function isClientDataRelevant(p: string): boolean {
  if (p.startsWith('components/') || p.startsWith('contexts/') || p.startsWith('lib/hooks/')) return true;
  if (/^app\/(?!api\/).+\.[jt]sx?$/.test(p)) return true;
  return /^lib\/[^/]+\/hooks\.[jt]sx?$/.test(p);
}
