...
describe('Foundation Setup first experience', () => {
  it('keeps the newest user message out of prior model history through a durable turn claim', () => {
    const repository = source('lib/api/repositories/onboarding.ts');
    expect(repository).toContain("rpc('begin_onboarding_turn'");
    expect(repository).toContain('history: result.history ?? scope.messages.map');
    expect(repository).toContain('conversationHistory: history');
  });
  ...
  it('keeps guided onboarding provisioning atomic and retryable', () => {
    const flow = source('components/onboarding/OnboardingFlow.tsx');
    ...
    expect(provisioner).toContain("rpc('provision_onboarding_session'");
    expect(provisioner).toContain('p_session_id: sessionData.id');
    expect(provisioner).toContain('p_automation_rows: withoutOrgId(automationRows)');
    expect(provisionRoute).toContain('sessionId');
  });
