...
export async function GET(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const DEFAULTS = { show_map: true, widgets: ['kpi_waci', 'sector_emissions'] as string[] };
  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ ...DEFAULTS, _hint: 'settings_error', _detail: error.message });
    }
    ...
    return NextResponse.json({ show_map, widgets });
  } catch (e: any) {
    // Network/other failure: return defaults
    return NextResponse.json({ ...DEFAULTS, _hint: 'settings_exception', _detail: e?.message || '' });
  }
}