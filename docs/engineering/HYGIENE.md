...
- `npm run verify:layout` prevents tracked local artifacts, retired paths,
  unapproved SQL locations, and unowned root-level library files.
- `npm run verify:hygiene` runs the layout, dead-code, and dependency checks and
  is part of repository CI.

## Local outputs

`npm run clean:local` removes ignored, regenerable build, walkthrough, test, and
tool outputs, plus macOS metadata files throughout the workspace. Use it when
the working tree becomes hard to navigate or before handing the repository to
another developer. It deliberately skips `.git` and `node_modules`, and leaves
source, database files, environment configuration, and dependencies alone.

Generated output must remain ignored. Do not commit `.next`, test reports,
Graphify output, TypeScript build info, or editor/OS metadata.

## Layout contract

Tracked SQL is limited to its canonical schema, bounded demo/seed data,
verification, and module-template locations. Coding-agent records belong under
`docs/agent-work/`; the previous agent-work location is retired. Root-level library files are
an explicit, temporary allowlist that narrows as ownership moves into domain
directories. Local Supabase branch and temporary state are never tracked.
...
`docs/engineering/ARCHITECTURE.md`, `docs/engineering/MODULES.md`, and
