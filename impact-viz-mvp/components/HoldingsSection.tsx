...
export default function HoldingsSection({ portfolioId, canEdit = false }: { portfolioId: string; canEdit?: boolean }) {
  ...
  return (
    <section className="space-y-4">
      ...
      {isLoading ? (
        <div className="rounded-2xl bg-white border border-black/5 shadow-soft p-6 text-sm text-neutral-500">Loading holdings…</div>
      ) : error ? (
        <div className="rounded-2xl bg-white border border-red-200 text-red-700 p-6 text-sm">{(error as any)?.message || 'Failed to load holdings'}</div>
      ) : (
        <HoldingsTable rows={rows as any} canEdit={canEdit} onEditRow={onEditRow} />
      )}
