...
describe('org-scoped route auth contracts', () => {
  ...
  it('acknowledgment routes use canonical columns and no-store responses', () => {
    ...
    expect(pdfRepository).toContain('storage.remove([storagePath])');
    expect(pdfRepository).toContain('upsert: false');
  ...
  it('org admin routes use no-store, soft delete, and durable audit for membership changes', () => {
    ...
    expect(membershipRepository).toContain("db.rpc('mutate_organization_membership'");
    expect(membershipRepository).not.toContain("from('org_audit_log').insert");
    expect(membershipRepository).not.toContain('countActiveOwners');
  ...
  it('invitation routes no-store admin data and commit delivery intent transactionally', () => {
    ...
    expect(cancelRoute).not.toContain('createAdminClient');
    expect(repository).toContain("operation: 'cancel'");
    expect(repository).toContain("operation: 'resend'");
  ...
  it('module and member collection routes no-store authority data and protect owner changes', () => {
    ...
    expect(membershipRepository).toContain("db.rpc('mutate_organization_membership'");
    expect(membershipRepository).toContain("mutate(userId, 'change_role', role)");
    expect(membershipRepository).not.toContain("from('organization_members').update");
  });
  ...
  it('custom-field routes use shared guards and one org-scoped repository', () => {
    ...
    // Automation is no longer invoked inline. The mutation records outbox event
    // ids in its own transaction and the repository drains them afterwards, so
    // a crash between write and automation retries instead of losing the event.
    expect(repository).toContain('drainCustomFieldAutomationOutbox');
    expect(repository).toContain('outbox_event_ids');
    const outbox = readFileSync('lib/tasks/automation/custom-field-outbox.ts', 'utf8');
    expect(outbox).toContain('runAutomationRulesForEvent');
  });
