...
- The canonical grant lifecycle parent is **`grants.id`** (migration 0041). `grant_details` is the old name — do not recreate it. `grant_milestones.grant_id`, `grant_reports.grant_id`, `grant_payments.grant_id`, and other grant ops tables reference `grants(id)`. `grants` carries `org_id` and `portfolio_id` directly, so grant ops can be scoped with `.eq('org_id', orgId)` on the `grants` table or via `grants!inner(org_id)` joins on child tables.
