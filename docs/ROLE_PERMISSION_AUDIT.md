...
authentication and `lib/roles.ts` for every role decision.
...
- `lib/roles.ts:1` defines `PortfolioRole = 'viewer'|'editor'|'owner'|'admin'`.
- `lib/roles.ts:5` defines `OrgRole = 'viewer'|'editor'|'admin'`, omitting `member` and `owner`.
...
- `lib/__tests__/org-routes.auth.test.ts:384` locks Builder routes around `is_org_admin`.
...
Replace `lib/roles.ts` with a canonical helper module used by app routes, components, schemas, and tests.
...
- Replace `lib/roles.ts` with canonical role constants, role ranking, and helper predicates.
