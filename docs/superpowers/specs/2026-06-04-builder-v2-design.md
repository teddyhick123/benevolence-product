...
`lib/builder/tools.ts:33` — the `update_module_config` tool's input schema has a hardcoded enum with 4 stale keys. Replace with all 10 canonical `ModuleId` values from `lib/modules/types.ts`:

```
core | impact_tracking | reporting | tax_optimization | grant_management |
...
case 'update_module_config': {
  InputValidator.validateString(toolInput.module, 'module', { maxLength: 64 });
  // then proceed
}
```
...
Executor: queries `organizations.modules` JSONB, merges with `MODULE_REGISTRY` to return full list with `enabled: boolean` per entry.
...
Migration: `db/migrations/0027_builder_events.sql`
...
CREATE INDEX builder_events_org_created_idx ON builder_events (org_id, created_at DESC);
CREATE INDEX builder_events_type_created_idx ON builder_events (event_type, created_at DESC);
```
...
CREATE VIEW v_builder_tool_usage AS
...
CREATE VIEW v_builder_ai_requests AS
...
Both views use `WITH (security_invoker = true)` and are readable only by `is_app_admin()`.
...
| File | Change |
|------|--------|
| `db/migrations/0027_builder_events.sql` | New migration |
...
3. For each file: create or update via `PUT /repos/{owner}/{repo}/contents/{path}` (content base64-encoded)
...
| `db/migrations/0026_builder_enhancement.sql` or new `0028` | Add `pr_url` column |
...
1. Track B migration (`0027_builder_events.sql`) — no code deps, deploy first
...
5. Track C (`github-apply.ts` + apply endpoint + `ReviewReportCard` button)
