...
export class OnboardingAssistant {
  ...
  async chat(params: {
    ...
    if (params.persist !== false) await this.updateSessionData(sessionId, extractions);
   ...
   */
  async generateRecommendations(sessionId: string, options: { persist?: boolean } = {}): Promise<{
    ...
    // Repository callers commit this with session state and analytics atomically.
    if (options.persist !== false) await this.supabase
