...
describe('Phase 4 org-specific AI context contract', () => {
  ...
  it('wires helper loading and prompt injection', () => {
    const helper = readFileSync('lib/organizations/ai-context.ts', 'utf8');
  ...
  it('captures and provisions onboarding context', () => {
    ...
    // The context rows no longer name the table in TypeScript: the repository
    // hands them to the provisioning RPC, which owns the org_ai_context insert
    // so that provisioning stays one transaction. Assert that whole chain.
    expect(provision).toMatch(/p_context_rows/);
    const provisioningRpc = readFileSync(
      'db/migrations/0056_onboarding_provisioning_recovery.sql',
      'utf8',
    );
    expect(provisioningRpc).toMatch(/p_context_rows/);
    expect(provisioningRpc).toMatch(/INSERT INTO public\.org_ai_context/);
  });
});
