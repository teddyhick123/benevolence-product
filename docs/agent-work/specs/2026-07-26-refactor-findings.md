...
[consolidated backlog](../BACKLOG.md) and maintain its
...
**Resolved 2026-08-13:** The historical measurement showed the serial job was
combining setup, smoke, and the deeper journey batch under one 30-minute
budget. The workflow now runs smoke and journeys as independent jobs, each
with its own seeded local stack, browser install, artifacts, and 30-minute
limit. This preserves complete coverage while removing the shared ceiling;
individual test failures remain independently observable.
...
**Resolved 2026-08-12:** Regeneration now writes a new private, versioned path
with `upsert: false`, commits that path before attempting to retire the prior
scoped object, and treats signed-URL failures as recoverable after the document
path is durable. The route and repository tests cover old-object preservation,
scoped retirement, and signing failure behavior.
...
**Resolved 2026-08-12:** `mutate_organization_membership` now serializes
owner-sensitive changes, locks the target membership, enforces actor and
last-owner rules, applies the mutation, and writes its audit event in one
transaction. The scoped repository calls only that RPC for mutations.
...
**Resolved 2026-08-12:** Invitation create, resend, and cancellation now use
one transactional function that commits invitation state, audit history, and a
durable email-outbox event. A scheduled, job-guarded worker claims events,
rejects stale tokens, and records sent, retry, cancelled, or terminal-failure
state without compensating invitation state after a delivery attempt.
...
**Resolved 2026-08-12:** The repository now validates and normalizes the
complete request before calling a service-only transactional mutation function.
That function applies every value change and creates one durable custom-field
automation event per change in the same commit. A worker claims, retries, and
idempotently settles those events; request-time delivery is only an
opportunistic drain. Clean-schema assertions prove a later invalid value rolls
back both prior values and events.
...
**Resolved 2026-08-12:** Invitation acceptance now uses a service-only,
token-bound transactional function. It locks and validates the invitation,
activates or restores the matching membership, marks the invitation accepted,
and writes the acceptance audit entry in one commit. Retries for the same user
and token are idempotent and do not duplicate audit history; a rotated token
cannot accept a resent invitation.
...
**Resolved 2026-08-12:** Onboarding now has normalized, append-only durable
turn and message rows keyed by `(user_id, request_id)`. Each user message is
claimed before model invocation; completed turns replay their stored response.
The assistant message, extracted profile data, conversation state, status
transition, and analytics commit together through one transaction, while a
failure records a durable failed turn without partial state. Recommendation
publication likewise atomically updates its result, session state, and
analytics.
...
**Resolved 2026-08-13:** `provision_onboarding_session` now uses the owned
`session_id` as a serialized idempotency key. It atomically creates or resumes
the organization, owner membership, owner portfolio membership, module state,
all onboarding blueprint configuration, analytics, and final session linkage.
Completed-session replays return the original setup without accepting a new
payload; a failed downstream configuration write rolls the entire setup back.
