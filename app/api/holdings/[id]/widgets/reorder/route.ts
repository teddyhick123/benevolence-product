// app/api/holdings/[id]/widgets/reorder/route.ts
//
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const access = await requireHoldingAccess(holding_id, 'member');
  ...
  const { data, error } = await access.context.db.rpc('swap_holding_widget_positions', {
